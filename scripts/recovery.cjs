...
async function main() {
  const { values, positionals } = parseArgs({ allowPositionals: true, strict: true, options: {
    database: { type: 'string' }, uploads: { type: 'string' }, output: { type: 'string' },
    backup: { type: 'string' }, offline: { type: 'boolean' }, help: { type: 'boolean' }
  } });
  if (values.help) {
    console.log('backup --database ABS --uploads ABS --output NUEVO_ABS --offline\nverify --backup ABS\nrestore --backup ABS --output NUEVO_ABS --offline\nTodas las rutas son explícitas. No se sobrescriben destinos. No se leen variables .env.');
    return;
  }
  const command = positionals[0];
  const allowed = { backup: ['database', 'uploads', 'output', 'offline'], verify: ['backup'], restore: ['backup', 'output', 'offline'] }[command];
  if (positionals.length !== 1 || !allowed || Object.keys(values).some(key => !allowed.includes(key)) || allowed.some(key => values[key] === undefined)) throw new Error('Argumentos incompletos o no admitidos. Consulta --help.');

...
it('labels the query honestly, preserves missing local and refreshes counts only through GET', async () => {
  ...
  expect(mocks.fetchWithAuth.mock.calls.filter(([url]) => !url.includes('/relevos/resumen'))).toHaveLength(18); expect(mocks.fetchWithAuth.mock.calls.every(([, options]) => !options?.method)).toBe(true);
});

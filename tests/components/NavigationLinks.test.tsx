...
it.each(['owner', 'manager'] as const)('keeps the complete existing management navigation for %s', role => {
  ...
  expect(screen.getByRole('button', { name: 'Informe mensual' })).toHaveAttribute('aria-current', 'page');
});
it('limits employee navigation to the five personal routes and optional settings', () => {
  ...
  expect(screen.getAllByRole('button').map(button => button.textContent?.trim())).toEqual(['Inicio', 'Relevo y rutinas', 'Mis turnos', 'Fichar', 'Buzón', 'Solicitudes', 'Ajustes']);
  expect(screen.queryByRole('button', { name: 'Personal y turnos' })).not.toBeInTheDocument();
});

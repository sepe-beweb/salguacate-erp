...
test('mobile navigation remains complete during panel failure, owns focus and closes on navigation or desktop resize', async ({ page }, testInfo) => {
  ...
  await expect(dialog.getByRole('button', { name: 'Inicio', exact: true })).toBeFocused();
  await expect(dialog.getByRole('button', { name: 'Gastos', exact: true })).toHaveCount(1);
  await expect(dialog.getByRole('button', { name: 'Personal y turnos' })).toHaveCount(1);
  ...
  await open.click(); await dialog.getByRole('button', { name: 'Personal y turnos' }).click();
  await expect(dialog).toHaveCount(0); await expect(page.getByRole('heading', { name: 'Personal y turnos' })).toBeVisible();
  await open.click(); await expect(dialog.getByRole('button', { name: 'Personal y turnos' })).toHaveAttribute('aria-current', 'page');
  ...
  await page.getByRole('navigation', { name: 'Navegación principal' }).getByRole('button', { name: 'Tareas' }).click();
  await expect(page).toHaveURL(/\/tareas$/); await page.setViewportSize({ width: 768, height: 900 }); await open.click();
  await dialog.getByRole('button', { name: 'Informe mensual' }).click(); await expect(page).toHaveURL(/\/informes$/); await expect(dialog).toHaveCount(0);
});
test('employee mobile menu offers only personal destinations and settings', async ({ page }, testInfo) => {
  ...
  const dialog = page.getByRole('dialog', { name: 'Navegación', exact: true });
  await expect(dialog.getByRole('navigation').getByRole('button')).toHaveCount(7);
  await expect(dialog.getByRole('button', { name: /Gastos|Personal y turnos|Informes/ })).toHaveCount(0);

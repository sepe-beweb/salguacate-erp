...
test('mobile catalogue and alerts share quantities, separate supplier identities and locals, and recover invalid reads', async ({ page, request }, testInfo) => {
  ...
  const equal = page.getByRole('region', { name: `Alertas de __proto__ · Aguacate · ${ids[0]}`, exact: true });
  await expect(equal).toContainText('En el mínimo (Min: 5)');
  await expect(page.getByRole('region', { name: `Alertas de __proto__ · Aguacate · ${ids[1]}`, exact: true })).toContainText('Hasta el mínimo: 3');

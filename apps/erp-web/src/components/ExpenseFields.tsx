...
export default function ExpenseFields({ value, onChange, disabled, amountLabel = 'Importe (€)', conceptRequired = false }: {
  ...
  return <fieldset disabled={disabled} className="space-y-3">
    ...
    <label className="block">Local<select value={value.local} onChange={e => onChange({ ...value, local: e.target.value })} className={inputClass}><option value="Principal">{locationLabel('Principal')}</option><option value="Segundo Local">{locationLabel('Segundo Local')}</option></select></label>

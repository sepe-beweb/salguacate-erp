...
// Business writes, audit and receipt share the same synchronous transaction.
function createOnce(db, req, operation, payload, status, create) {
  ...
  return db.transaction(sql => {
    if (key) {
      const receipt = sql.prepare('SELECT request_hash, status, response_json FROM idempotency_requests WHERE actor_id = ? AND operation = ? AND request_key = ?').get(req.user.id, operation, key);
      if (receipt) {
    ...
    const body = create(sql);
    if (key) sql.prepare('INSERT INTO idempotency_requests (actor_id, operation, request_key, request_hash, status, response_json) VALUES (?, ?, ?, ?, ?, ?)').run(req.user.id, operation, key, fingerprint, status, JSON.stringify(body));
    return { status, body };
  });
}

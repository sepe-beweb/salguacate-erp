...
describe('Fresh installation bootstrap with disposable databases', () => {
  it('creates one usable owner with a hash and no sample business rows, then refuses a second bootstrap', async () => {
    ...
    try {
      ...
      for (const table of ['fichajes','proveedores','inventario','turnos','mensajes','eventos','notas','cierres','gastos','tareas','pedidos','peticiones','idempotency_requests','rutinas','rutina_ejecuciones','relevos','relevo_lecturas','relevo_gestion','relevo_cambios']) expect(db.connection.prepare(`SELECT count(*) AS n FROM ${table}`).get().n).toBe(0);
